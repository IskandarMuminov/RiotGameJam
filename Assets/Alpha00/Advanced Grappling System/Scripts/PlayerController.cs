...
public class PlayerController : MonoBehaviour {
    ...
    public GameObject speedSoundSource;
    ...
    void Start()
    {
        myCam = myCam.GetComponent<Camera>();
        m_FieldOfView = myCam.fieldOfView;
    ...
    private void FixedUpdate() {
        ...
        if (vel.magnitude >= maxSpeed + speedVisual)
        {
            speedParticle.SetActive(true);
            speedSoundSource.SetActive(true);
            m_FieldOfView = 60.0f;
        }
        else
            {
            speedSoundSource.SetActive(false);
